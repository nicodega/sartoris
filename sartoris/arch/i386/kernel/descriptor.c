...
void init_desc_tables() {
  ...
  gdt[3].dword1 = D_DW1_BASE(0xa0000) | D_DW1_LIMIT(0x60000) | 
   ...
   /* new gdt load */
  l_desc[0] = 
      ( GDT_ENT * sizeof(struct seg_desc) + ((((unsigned int) gdt) & 0xffff) << 16) );
...
void init_tss_desc(int id) {
  ...
  i = GDT_TSS + id;
  tss_adr = (unsigned int) &thr_tss[id];
  gdt[i].dword0 = D_DW0_BASE(tss_adr) + D_DW0_LIMIT(sizeof(struct tss));
  gdt[i].dword1 = D_DW1_BASE(tss_adr) + D_DW1_LIMIT(sizeof(struct tss)) + perms;
}
