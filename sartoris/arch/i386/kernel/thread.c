...
int arch_run_thread_int(int id, void *eip, void *stack)
{
	...
	*/
	...
	return arch_switch_thread_int(GET_THRSTATE_ARCH(id), id, (unsigned int)page_dir_base, eip, stack);
#else
	return arch_switch_thread_int(GET_THRSTATE_ARCH(id), id, 0, eip, stack);
#endif
}
