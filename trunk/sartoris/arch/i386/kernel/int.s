...
exceptional_death:
	...
	popf
	popa
	call arch_dump_cpu	; this is broken! there is no way to pass
				; the correct segment registers to the 
				; printing routine. arch_dump_cpu has to be
				; re-written to take a pointer to some
				; cpu-state-like structure (?).  
	mov ebp, [esp]		; NO. we must poke in the stack, it's all
	mov ebx, [esp+4]	; there.
	mov esi, [esp+8]
	mov edi, [esp+12]
