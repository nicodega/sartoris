...
void process_manager(void) 
{
    ...
    open_port(FILESYS_INPUT_PORT, 0, UNRESTRICTED);
    ...
    for(;;) 
    {
            ...
            if (0 <= i && i < NUM_PROC)
            {
                switch(state[i])
                {
                    case LOADING:
                        ...
                        if (fs_res.result<0) 
                        {
                            fetch_file(i, FS_TASK, FILESYS_INPUT_PORT);
                        } 
                        else 
                        {
                            do_load(i);
        ...
        /* get acks from floppy filesystem */
        while (get_msg_count(FILESYS_INPUT_PORT) > 0) 
        {
            get_msg(FILESYS_INPUT_PORT, &fs_res, &id);
            i=fs_res.id;
            if (0 <= i && i < NUM_PROC)
            {
                switch(state[i])
                {
                    case LOADING:
                        ...
                        {
                            inform_load_error(i, fs_res.result);
                            show_prompt(i);
                        } 
                        else 
                        {
